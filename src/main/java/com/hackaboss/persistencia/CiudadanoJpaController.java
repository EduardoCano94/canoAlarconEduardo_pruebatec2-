...
public class CiudadanoJpaController implements Serializable {
    ...
    public void edit(Ciudadano ciudadano) throws IllegalOrphanException, NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            for (Turno turnosOldTurno : turnosOld) {
                if (!turnosNew.contains(turnosOldTurno)) {
                    ...
                    illegalOrphanMessages.add("You must retain Turno " + turnosOldTurno + " since its ciudadano field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
    ...
    public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            List<Turno> turnosOrphanCheck = ciudadano.getTurnos();
            for (Turno turnosOrphanCheckTurno : turnosOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Ciudadano (" + ciudadano + ") cannot be destroyed since the Turno " + turnosOrphanCheckTurno + " in its turnos field has a non-nullable ciudadano field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
